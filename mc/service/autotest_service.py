...
class AutoTest:
    def __init__(self, suite, name, to_user, case_type, env):
        self.suite = suite
        self.name = name
        self.to_user = to_user
        self.case_type = case_type
        self.env = env
        self.data_from = tt_config.DATA_FROM
    ...
    def run(self):
        ...
        # 存储测试集执行结果
        result_set_sql = "INSERT INTO ui_autotest_set_record(testName, dataFrom, env, testAll, testPass, testFail, testError, testSkip, " \
                         "totalTime, testTime, setId, reportHtml) VALUES('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', " \
                         "'%s', '%s', '%s')"
        ...
        conn.alter(result_set_sql % (testName, self.data_from, self.env, testAll, testPass, testFail, testError,
                                     testSkip, totalTime, testTime, setId, reportHtml))

        # 获取case执行的详细结果
        result_cases = result_set['testResult']
        # 存储case执行结果
        result_case_sql = "INSERT INTO ui_autotest_case_record(setId, className, caseName,caseType, dataFrom, env, description, spendTime, caseStatus, " \
        ...
        for result_case in result_cases:
            ...
            conn.alter(
                result_case_sql % (
                setId, className, caseName, self.case_type, self.data_from, self.env, description, spendTime, caseStatus, caseLog, author,
